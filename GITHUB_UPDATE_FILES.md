...
## 1. server/routes-minimal.ts (COMPLETE FILE)
...
## 2. netlify.toml (UPDATE BUILD SETTINGS)

Make sure your netlify.toml file uses the minimal routes:
...
## 3. vite.config.prod.ts (VERIFY BUILD CONFIG)
...
## Deployment Steps After GitHub Update

1. **Push these files to GitHub**
2. **Wait for Netlify to rebuild** (check deployment status)
3. **Initialize database**: `curl.exe -Uri "https://PNLToolkit.professordtreku.com/api/init-db" -Method POST`
4. **Seed admin accounts**: `curl.exe -Uri "https://PNLToolkit.professordtreku.com/api/seed" -Method POST`
5. **Test login**: 
   ```powershell
   curl.exe -Uri "https://PNLToolkit.professordtreku.com/api/auth/login" -Method POST -ContentType "application/json" -Body '{"email":"dtreku@wpi.edu","password":"admin123"}'
   ```
